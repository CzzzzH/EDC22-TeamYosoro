...
public:
    ...
    void setMagBias(uint8_t i, float v)
    {
        if (i < 3)
            magBias[i] = v;
    }
    void setMagScale(uint8_t i, float v)
    {
        if (i < 3)
            magScale[i] = v;
    }
...
private:
    ...
    void magcalMPU9250(float *dest1, float *dest2)
    {
        ...
        if (Mmode == 0x02)
            sample_count = 32; // at 8 Hz ODR, new mag data is available every 125 ms
        else if (Mmode == 0x06)
            sample_count = 375; // at 100 Hz ODR, new mag data is available every 10 ms
        ...
        for (ii = 0; ii < sample_count; ii++)
        {
            ...
            if (Mmode == 0x02)
                delay(126); // at 8 Hz ODR, new mag data is available every 125 ms
            if (Mmode == 0x06)
